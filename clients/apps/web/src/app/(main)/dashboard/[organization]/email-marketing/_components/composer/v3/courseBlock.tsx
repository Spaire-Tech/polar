...
function editorSpec(variant: CourseVariant, c: CourseData): Spec {
  ...
  switch (variant) {
    ...
    case 'welcome':
      return wrap(
        'padding:56px 44px',
        ['h2', { style: `margin:0 0 24px;font-size:34px;font-weight:700;line-height:1.1;letter-spacing:-.5px;color:${H}` }, 'Welcome to the table.'],
...
function emailEl(variant: CourseVariant, c: CourseData): ReactNode {
  switch (variant) {
    ...
    case 'welcome':
      return div(
        { padding: '56px 44px' },
        txt('h2', { margin: '0 0 24px', fontSize: 34, fontWeight: 700, lineHeight: 1.1, color: eH }, 'Welcome to the table.'),
        ...c.welcome.map((p, i) =>
