  ...
  return (
    <div className="@container flex flex-col gap-2">
      <div className="flex flex-col gap-4">
        ...
        {llmMetadata && (
          <div className="flex flex-row gap-4 rounded-2xl bg-white/[0.04] px-4 pt-3.5 pb-4 text-sm text-xs">
            <dl className="flex flex-row items-center gap-4">
              <div className="flex flex-row items-center gap-1 font-mono">
                <dt>
                  <BotIcon className="size-3 text-polar-500" />
                </dt>
                <dd>
              ...
              <div className="flex flex-row items-center gap-1 font-mono">
                <dt>
                  <ArrowUpFromLineIcon className="size-3 text-polar-500" />
                </dt>
              ...
              <div className="flex flex-row items-center gap-1 font-mono">
                <dt>
                  <ArrowDownFromLineIcon className="size-3 text-polar-500" />
                </dt>
                <dd>{llmMetadata.output_tokens}</dd>
              </div>
              <div className="flex flex-row items-center gap-1 font-mono">
                <dt>
                  <SigmaIcon className="size-3 text-polar-500" />
                </dt>
  ...
  return (
    <div className="group relative flex gap-x-4">
      <div className="relative flex w-3 justify-center">
        <div className="absolute top-[10px] z-10 h-3 w-3 rounded-full bg-polar-500" />
        {!isLast && (
          <div className="absolute top-[16px] bottom-[-16px] w-0.5 bg-white/[0.08]" />
        )}
      </div>
          ...
          <div className="flex flex-row items-center justify-between gap-2">
            <div className="flex flex-row items-center gap-2">
              ...
              <div className="flex flex-row items-center gap-2">
                ...
                <span className="font-mono text-xs text-polar-500">
                  +{formattedTimestamp}
                </span>
              </div>
            </div>
            ...
            {costDisplay && (
              <span className="font-mono text-sm text-polar-500 tabular-nums transition-colors duration-150 group-hover:text-polar-300">
                {costDisplay}
              </span>
            )}
          </div>
        ...
        >
          ...
          >
            {event.label === eventType.label ? (
              <span className="truncate font-mono font-normal text-polar-200">
                {event.id}
              </span>
            ) : (
              event.label
