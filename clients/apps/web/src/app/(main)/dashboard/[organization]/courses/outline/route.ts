...
export async function POST(req: Request) {
  ...
  const structureRule = architectProposal
    ? 'Reproduce the ARCHITECT PROPOSAL: one module per season, its episodes as lessons, in order. If a season has more than 6 episodes, keep them all anyway — the skeleton wins over the 6-lesson cap.'
    : isSeries
