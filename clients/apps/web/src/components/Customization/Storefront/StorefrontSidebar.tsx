    ...
    <div className="flex flex-col gap-y-8 px-8 py-8">
      ...
      <div className="flex flex-col gap-y-3">
        ...
        {isEnabled && (
          ...
          <ShareSpaceButtons spaceUrl={spaceUrl} orgName={organization.name} />
        )}
      </div>
...
function ShareSpaceButtons({ spaceUrl, orgName }: { spaceUrl: string; orgName: string }) {
  const [copiedMsg, setCopiedMsg] = useState(false)

  const shareText = `I just launched my Spaire Space\n\nAll my work is up here now!\n${spaceUrl}`
  ...
  const twitterUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(`I just launched my Spaire Space\n\nAll my work is up here now!`)}&url=${encodeURIComponent(spaceUrl)}`
  const linkedinUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(spaceUrl)}`
