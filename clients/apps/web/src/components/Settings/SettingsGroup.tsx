...
export const SettingsGroup: React.FC<React.PropsWithChildren> = ({
  ...
  <div className="w-full flex-col divide-y divide-white/[0.06] overflow-hidden rounded-2xl bg-white/[0.04] ring-1 ring-white/[0.08]">
    {children}
  </div>
)
...
> = ({ children, title, description, vertical }) => (
  ...
  >
    <div className="flex w-full flex-col md:max-w-1/2">
      ...
      {description && (
        <p className="text-xs text-polar-500">
          {description}
        </p>
      )}
    </div>
