  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          type="button"
          className={`dark:border-spaire-700 dark:bg-spaire-800 dark:text-spaire-300 dark:hover:bg-spaire-700 flex h-10 items-center justify-between gap-2 rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 shadow-xs hover:bg-gray-50 ${className ?? ''}`}
        >
      ...
      >
        <div className="border-b p-2">
          <input
            ...
            className="dark:bg-spaire-800 dark:text-spaire-200 dark:placeholder:text-spaire-500 w-full rounded-md border-0 bg-gray-50 px-3 py-2 text-sm outline-none placeholder:text-gray-400"
          />
        </div>
        <div className="dark:border-spaire-700 flex items-center justify-between border-b px-3 py-2">
        ...
        <div className="max-h-64 overflow-y-auto p-1">
          {filteredEventTypes.length > 0 ? (
            filteredEventTypes.map((eventType) => (
              <label
                key={eventType}
                className="dark:hover:bg-spaire-700 flex cursor-pointer items-center gap-3 rounded-md px-2 py-2 hover:bg-gray-50"
              >
                <Checkbox
            ...
            <div className="dark:text-spaire-500 py-4 text-center text-sm text-gray-500">
