...
const SocialIcon = ({
  ...
  platform: schemas['OrganizationSocialPlatforms']
}) => {
  switch (platform) {
    case 'x':
      return (
...
export const StorefrontHeader = ({
  ...
  return (
    <div className="dark:border-polar-700 dark:bg-polar-900 flex w-full flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white">
      {/* Banner */}
      {showHeader && (
        <div className="relative w-full bg-gray-100 dark:bg-black" style={{ aspectRatio: '16 / 6' }}>
          {headerImageUrl ? (
      ...
      <div className="flex flex-col gap-y-4 p-6">
        {/* Avatar — overlaps banner if shown */}
        {showLogo && (
          <div className={showHeader ? '-mt-14' : ''}>
            <Avatar
              className="dark:border-polar-900 h-20 w-20 border-4 border-white text-lg md:text-2xl"
        ...
        {/* Powered by Spaire */}
        <div className="mt-4 flex flex-row items-center gap-x-2 border-t border-gray-100 pt-4 dark:border-polar-700">
