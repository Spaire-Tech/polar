...
- [With Node.js](https://github.com/spairehq/examples/tree/main/with-nodejs)
...
const spaire = new Spaire({
  ...
  server: process.env.SPAIRE_MODE || 'sandbox' // sandbox or production
})

async function run() {
  const result = await spaire.users.benefits.list({})
