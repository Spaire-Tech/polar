...
- [With TanStack Start](https://github.com/spairehq/examples/tree/main/with-tanstack-start)
...
<Tabs>
  ...
  <Tab title="bun">
    ```bash Terminal
    bun add zod @spaire/tanstack-start
    ```
  </Tab>
</Tabs>
  ...
  server: {
    handlers: {
      GET: CustomerPortal({
        ...
        getCustomerId: async (request: Request) => "", // Function to resolve a Spaire Customer ID
        returnUrl: "https://myapp.com", // An optional URL which renders a back-button in the Checkout
        server: "sandbox", // Use sandbox if you're testing Spaire - omit the parameter or pass 'production' otherwise
      }),
    },
...
```typescript icon="square-js" routes/api/webhook/spaire.ts
import { Webhooks } from "@spaire/tanstack-start";
import { createFileRoute } from "@tanstack/react-router";

export const Route = createFileRoute("/api/webhook/spaire")({
  server: {
    handlers: {
      POST: Webhooks({
