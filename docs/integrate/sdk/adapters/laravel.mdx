...
Seamlessly integrate Spaire.sh subscriptions and payments into your Laravel application. This package provides an elegant way to handle subscriptions, manage recurring payments, and interact with Spaire's API. With built-in support for webhooks, subscription management, and a fluent API, you can focus on building your application while we handle the complexities of subscription billing.

<Note>
  This provider is not maintained or officially supported by Spaire. Use at your own discretion. If you have questions about the provider, please contact the project maintainer.
</Note>
...
```bash Terminal
composer require danestves/laravel-spaire
```

**Step 2:** Run `:install`:

```bash Terminal
php artisan spaire:install
```
...
```bash Terminal
php artisan vendor:publish --tag="spaire-migrations"
```

```bash Terminal
php artisan vendor:publish --tag="spaire-config"
```

```bash Terminal
php artisan vendor:publish --tag="spaire-views"
```
...
return [
    ...
    |--------------------------------------------------------------------------
    |
    | The Spaire access token is used to authenticate with the Spaire API.
    | You can find your access token in the Spaire dashboard > Settings
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Spaire Webhook Secret
    |--------------------------------------------------------------------------
    |
    | The Spaire webhook secret is used to verify that the webhook requests
    | are coming from Spaire. You can find your webhook secret in the Spaire
    ...
    |
    */
    'webhook_secret' => env('SPAIRE_WEBHOOK_SECRET'),
    ...
    |--------------------------------------------------------------------------
    |
    | This is the base URI where routes from Spaire will be served
    | from. The URL built into Spaire is used by default; however,
    | you can modify this path as you see fit for your application.
    |
    */
    'path' => env('SPAIRE_PATH', 'spaire'),
    ...
    | from a checkout session in your Spaire account.
    |
    */
    'redirect_url' => null,
    ...
    |
    */
    'currency_locale' => env('SPAIRE_CURRENCY_LOCALE', 'en'),
];
```
...
Configure your access token. Create a new token in the Spaire Dashboard \> Settings \> Developers and paste it in the `.env` file.

- [https://sandbox.spairehq.com/dashboard/ORG\_SLUG/settings](https://sandbox.spairehq.com/dashboard/ORG_SLUG/settings) (Sandbox)
- [https://www.spairehq.com/dashboard/ORG\_SLUG/settings](https://www.spairehq.com/dashboard/ORG_SLUG/settings) (Production)
...
Configure your webhook secret. Create a new webhook in the Spaire Dashboard \> Settings \> Webhooks.

- [https://sandbox.spairehq.com/dashboard/ORG\_SLUG/settings/webhooks](https://sandbox.spairehq.com/dashboard/ORG_SLUG/settings/webhooks) (Sandbox)
- [https://www.spairehq.com/dashboard/ORG\_SLUG/settings/webhooks](https://www.spairehq.com/dashboard/ORG_SLUG/settings/webhooks) (Production)
...
Spaire includes a JavaScript script that you can use to initialize the [Spaire Embedded Checkout](https://www.spairehq.com/docs/features/checkout/embed). If you going to use this functionality, you can use the `@spaireEmbedScript` directive to include the script in your views inside the `<head>` tag.
...
This package includes a webhook handler that will handle the webhooks from Spaire.
...
Or if you're using Laravel v11 and up, you should exclude `spaire/*` in your application's `bootstrap/app.php` file:
...
This package includes a list of commands that you can use to retrieve information about your Spaire account.

| Command | Description |
| --- | --- |
| `php artisan spaire:products` | List all available products with their ids |
...
```blade
<x-spaire-button :checkout="$checkout" />
```
...
```blade
<x-spaire-button :checkout="$checkout" data-spaire-checkout-theme="dark" />
```
...
```php
public function spaireName(): ?string; // default: $model->name
public function spaireEmail(): ?string; // default: $model->email
```
...
Customers can update their personal information (e.g., name, email address) by accessing their [self-service customer portal](https://www.spairehq.com/docs/features/customer-portal). To redirect customers to this portal, call the `redirectToCustomerPortal()` method on your billable model (e.g., the User model).
...
```blade
@if ($order->refunded())
    Order {{ $order->spaire_id }} was refunded on {{ $order->refunded_at->toFormattedDateString() }}
@endif
```
...
Spaire does not offer immediate cancellation. To resume a subscription while it is still in its grace period, use the resume method.
...
Spaire can send webhooks to your app, allowing you to react. By default, this package handles the majority of the work for you. If you have properly configured webhooks, it will listen for incoming events and update your database accordingly. We recommend activating all event kinds so you may easily upgrade in the future.

#### Webhook Events

- `Danestves\LaravelSpaire\Events\BenefitGrantCreated`
- `Danestves\LaravelSpaire\Events\BenefitGrantUpdated`
- `Danestves\LaravelSpaire\Events\BenefitGrantRevoked`
- `Danestves\LaravelSpaire\Events\OrderCreated`
- `Danestves\LaravelSpaire\Events\OrderRefunded`
- `Danestves\LaravelSpaire\Events\SubscriptionActive`
- `Danestves\LaravelSpaire\Events\SubscriptionCanceled`
- `Danestves\LaravelSpaire\Events\SubscriptionCreated`
- `Danestves\LaravelSpaire\Events\SubscriptionRevoked`
- `Danestves\LaravelSpaire\Events\SubscriptionUpdated`
...
use Danestves\LaravelSpaire\Events\WebhookHandled;

class SpaireEventListener
{
    /**
     * Handle received Spaire webhooks.
     */
...
The [Spaire documentation](https://www.spairehq.com/docs/integrate/webhooks/events) includes an example payload.
...
use App\Listeners\SpaireEventListener;
...
Please see [CHANGELOG](https://github.com/danestves/laravel-spaire/blob/main/CHANGELOG.md) for more information on what has changed recently.

## License

The MIT License (MIT). Please see [License File](https://github.com/danestves/laravel-spaire/blob/main/LICENSE.md) for more information.