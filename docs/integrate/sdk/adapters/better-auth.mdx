...
A [Better Auth](https://github.com/better-auth/better-auth) plugin for integrating [Spaire](https://www.spairehq.com) payments and subscriptions into your authentication flow.
...
- [Handle Spaire Webhooks securely with signature verification](#webhooks-plugin)
- [Customer Portal](#portal-plugin)

## Examples

- [With Next.js, Better Auth and Cloudflare Workers](https://github.com/spairehq/examples/tree/main/with-nextjs-better-auth-cloudflare-workers)

## Installation

Install the required Better Auth and Spaire packages using the following command:
...
<Tabs>
  ...
  <Tab title="bun">
    ```bash Terminal
    bun add better-auth @spaire/better-auth @spaire/sdk
    ```
  </Tab>
</Tabs>

## Integrate Spaire with BetterAuth

<Steps>
  <Step title="Configure Spaire Access Token">
    Go to your Spaire Organization Settings, create an Organization Access Token, and add it to the environment variables of your application.
  ...
  <Step title="Configure BetterAuth Server">
    The Spaire plugin comes with it's own set of plugins to add functionality to your stack:
    ...
    ```typescript icon="square-js" BetterAuth Server with Spaire Example
    import { betterAuth } from "better-auth";
    import { spaire, checkout, portal, usage, webhooks } from "@spaire/better-auth"; // [!code ++]
    import { Spaire } from "@spaire/sdk"; // [!code ++]

    const spaireClient = new Spaire({ // [!code ++]
        accessToken: process.env.SPAIRE_ACCESS_TOKEN, // [!code ++]
    ...
    const auth = betterAuth({
        ...
        plugins: [
            spaire({ // [!code ++]
                client: spaireClient, // [!code ++]
                createCustomerOnSignUp: true, // [!code ++]
                use: [ // [!code ++]
                    checkout({ // [!code ++]
                        products: [ // [!code ++]
                            { // [!code ++]
                                productId: "123-456-789", // ID of Product from Spaire Dashboard // [!code ++]
                    ...
                    usage(), // [!code ++]
                    webhooks({ // [!code ++]
                        secret: process.env.SPAIRE_WEBHOOK_SECRET, // [!code ++]
    ...
    #### Spaire Plugin Configuration Options
    ...
    const auth = betterAuth({
      ...
      plugins: [
        spaire({
          ...
          use: [ // [!code ++]
            // This is where you add Spaire plugins // [!code ++]
          ], // [!code ++]
        }),
      ],
    });
    ...
    - `client` (required): Spaire SDK client instance
    - `createCustomerOnSignUp` (optional): Automatically create a Spaire customer when a user signs up
    - `getCustomerCreateParams` (optional): Custom function to provide additional customer creation metadata
    - `use` (optional): Array of Spaire plugins to enable specific functionality (checkout, portal, usage, and webhooks)
  ...
  <Step title="Configure BetterAuth Client">
    ...
    ```typescript icon="square-js" BetterAuth Client with Spaire Example
    import { createAuthClient } from "better-auth/react";
    import { spaireClient } from "@spaire/better-auth/client"; // [!code ++]
    import { organizationClient } from "better-auth/client/plugins"; // [!code ++]

    // All Spaire plugins, etc. should be attached to BetterAuth server
    export const authClient = createAuthClient({ // [!code ++]
      plugins: [spaireClient()], // [!code ++]
    }); // [!code ++]
    ```
...
Enable the `createCustomerOnSignUp` [Spaire plugin configuration option](#spaire-plugin-configuration-options) to automatically create a new Spaire Customer when a new User is added in the BetterAuth database.

All new customers are created with an associated `externalId`, i.e. the ID of your User in the Database. This skips any Spaire to User mapping in your database.

## Sync Customer deletion

To add user deletion logic with external Spaire customer deletion in BetterAuth, extend the user config of your betterAuth setup with the deleteUser option and an afterDelete hook. Here’s how to integrate this alongside your Spaire plugin and automatic customer creation:
...
[Source code](https://github.com/spairehq/spaire-adapters/blob/main/packages/spaire-betterauth/src/plugins/checkout.ts)
...
<Steps>
  <Step title="Use Checkout Plugin">
    Update the `use` property of the Spaire plugin for BetterAuth client to have the `checkout` plugin.
  ...
  <Step title="Create checkouts using BetterAuth client">
    ...
    - **`products`** (optional): An array of Spaire Product IDs.
    ...
    ```typescript icon="square-js" BetterAuth Checkout with Spaire Example
    ...
    ```typescript icon="square-js" BetterAuth Checkout with Spaire Organization Example
...
[Source code](https://github.com/spairehq/spaire-adapters/blob/main/packages/spaire-betterauth/src/plugins/usage.ts)
...
[Source code](https://github.com/spairehq/spaire-adapters/blob/main/packages/spaire-betterauth/src/plugins/webhooks.ts)

The `webhooks` plugin can be used to capture incoming events from your Spaire organization.

To set up the Spaire `webhooks` plugin with the BetterAuth client, follow the steps below:

<Steps>
  <Step title="Configure Webhook Endpoints in Spaire">
    Configure a Webhook endpoint in your Spaire Organization Settings page by following [this guide](/integrate/webhooks/endpoints). Webhook endpoint is configured at `/api/auth/spaire/webhooks`.
  </Step>
  <Step title="Add the Webhook Secret">
    Add the obtained webhook secret to your application environment as an environment variable (to be used as `process.env.SPAIRE_WEBHOOK_SECRET`):
...
The `webhooks` plugin allows you to invoke handlers for all Spaire webhook events:
...
[Source code](https://github.com/spairehq/spaire-adapters/blob/main/packages/spaire-betterauth/src/plugins/portal.ts)
...
The following method will redirect the user to the Spaire Customer Portal, where they can see their orders, purchases, subscriptions, benefits, etc.
...
Learn more about the Spaire Customer State [in the Spaire Docs](https://www.spairehq.com/docs/integrate/customer-state).
