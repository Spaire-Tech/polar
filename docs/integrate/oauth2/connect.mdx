...
```
https://www.spairehq.com/oauth2/authorize?
  response_type=code
  &client_id=CLIENT_ID
...
<ParamField path="response_type=code" type="string" required>
  Indicates that you want to use the authorization code flow. Most common and the only one supported by Spaire.
</ParamField>
...
If you redirect the user to this URL, they'll see a page asking them to grant access to their data, corresponding to the scopes you asked for. By default, the user will also be prompted to select one of their organizations, as Spaire generates organization-level access tokens by default.
...
```bash Terminal
curl -X POST https://api.spairehq.com/v1/oauth2/token \
...
{
  "token_type": "Bearer",
  "access_token": "spaire_at_XXX",
  "expires_in": 864000,
  "refresh_token": "spaire_rt_XXX",
  "scope": "openid email",
  "id_token": "ID_TOKEN"
}
```
...
By default, Spaire OAuth2 flow generates **organization-level** access tokens. These tokens are tied to a specific organization rather than a user, allowing requests to operate only on that organization's data, which improves privacy and security.
...
```
https://www.spairehq.com/oauth2/authorize?response_type=code&client_id=spaire_ci_j3X95_MgfdSCeCd2qkFnUw&redirect_uri=https%3A%2F%2Fexample.com%2Fcallback&scope=openid%20email
```
...
```
https://www.spairehq.com/oauth2/authorize?response_type=code&client_id=spaire_ci_j3X95_MgfdSCeCd2qkFnUw&redirect_uri=https%3A%2F%2Fexample.com%2Fcallback&scope=openid%20email&sub_type=user
```
...
```bash Terminal
curl -X GET https://api.spairehq.com/v1/oauth2/userinfo \
  -H 'Authorization: Bearer spaire_at_XXX'
```