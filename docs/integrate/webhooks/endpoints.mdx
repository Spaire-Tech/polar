...
<Steps>
  ...
  <Step title="Specify your endpoint URL">
    ...
    <Tip>
      **Developing locally?**
      ...
      Just be sure to provide the URL ngrok gives you as the webhook endpoint on Spaire.
    </Tip>
  </Step>
  <Step title="Choose a delivery format">
    ...
    If you wish to send notifications to a Discord or Slack channel, you can select the corresponding format here. Spaire will then adapt the payload so properly formatted messages are sent to your channel.
  ...
  <Step title="Set a secret">
    We cryptographically sign the requests using this secret. So you can easily verify them using our SDKs to ensure they are legitimate webhook payloads from Spaire.
