...
description: Stay up to date with the latest changes and improvements to Spaire.
---

<Update label="2025-10-17">
  ## Ability to update subscription to an updated price of the product

  Merchants can now [update existing subscriptions](https://www.spairehq.com/docs/api-reference/subscriptions/update#subscriptionupdateproduct) from archived pricing schemes to current ones within the same product.
  ...
  From **October 27th, 2025**, [new IP ranges](https://www.spairehq.com/docs/integrate/webhooks/delivery#ip-allowlist) will be added.

  ## Improved Subscription Cancellation Flow

  [Benefits](https://www.spairehq.com/docs/features/benefits/introduction) attached to the subscription [are now automatically revoked](https://github.com/spairehq/spaire/pull/7271) when the subscription is canceled.

  ## Ability to specify External ID

  [External ID can now be specified](https://github.com/spairehq/spaire/pull/7275) during creation of a customer via the dashboard.
  ...
  A Return URL can now be set while generating a [checkout session](https://www.spairehq.com/docs/api-reference/checkouts/create-session#body-return-url-one-of-0) or a [customer portal session](https://www.spairehq.com/docs/api-reference/customer-portal/sessions/create#body-one-of-0-return-url-one-of-0). This allows you to preserve the context for the end users who visit either if they wish to go back to the application.

  ## Ability to search Customers via their External ID

  The [List Customers API](https://www.spairehq.com/docs/api-reference/customers/list) now accepts Customer's External ID [in the **query** parameter](https://www.spairehq.com/docs/api-reference/customers/list#parameter-one-of-0).
...
We're excited to introduce a chat widget to the Spaire dashboard, making it easier than ever for merchants to get help directly within the product.
...
We've updated our webhooks server implementation to [include a timestamp in each payload](https://github.com/spairehq/spaire/pull/6770), in line with the Standard Webhooks specification.
...
Customer emails now appear to come from your organization (e.g., "YourOrg (via Spaire)") with replies directed to your organization's email address, providing a more professional and branded experience for your customers.
...
We're excited to announce the official launch of our [Zapier integration](https://zapier.com/apps/spaire/integrations)! Get started now and connect Spaire to 2,000+ other web services.

<Note>
  We've focused on **triggers** (webhooks) for now, so you can react to events in Spaire and trigger actions in other apps.

Need to perform actions in Spaire? Tell us about your use case [here](https://github.com/orgs/spairehq/discussions/new?category=integrations&labels=integrations%2Fzapier) and we'll consider adding more actions in the future.
...
thrilled to have shipped a Spaire plugin for Better Auth - in collaboration with them.
...
We've added support for an `external_id` field on Customers. We believe this will greatly simplify the reconciliation process between your system and Spaire.
...
With `external_id`, you can now fetch a Customer directly by their external ID through dedicated `GET`, `PATCH`, and `DELETE` endpoints. You don't even need to store Spaire's internal ID in your system anymore! [Read more](/features/customer-management#external-id)
...
<Update label="2025-02-19">
  ## Spaire's take on Product variants
