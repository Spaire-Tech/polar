...
<CardGroup cols={2}>
  <Card title="Production Base URL" icon="globe">
    `https://api.spairehq.com/v1`
  </Card>
  <Card title="Sandbox Base URL" icon="flask">
    `https://sandbox-api.spairehq.com/v1`
  </Card>
...
| ----------- | --------------------------------- | ------------------------------- |
| Production  | `https://api.spairehq.com/v1`         | Real customers & live payments  |
| Sandbox     | `https://sandbox-api.spairehq.com/v1` | Safe testing & integration work |
...
```http
Authorization: Bearer spaire_oat_xxxxxxxxxxxxxxxxx
```
...
curl https://api.spairehq.com/v1/products/ \
  -H "Authorization: Bearer $SPAIRE_OAT" \
  -H "Accept: application/json"
```

```bash curl (Sandbox - Core API)
curl https://sandbox-api.spairehq.com/v1/products/ \
  -H "Authorization: Bearer $SPAIRE_OAT_SANDBOX" \
  -H "Accept: application/json"
```

```bash curl (Customer Portal API)
curl https://api.spairehq.com/v1/customer-portal/orders/ \
  -H "Authorization: Bearer $SPAIRE_CUSTOMER_TOKEN" \
  -H "Accept: application/json"
```
...
```py Python
from spaire import Spaire

client = Spaire(
    access_token=os.environ["SPAIRE_ACCESS_TOKEN"],
    server="sandbox",
)
````
...
List endpoints in the Spaire API support pagination to help you efficiently retrieve large datasets. Use the `page` and `limit` query parameters to control pagination.
...
<CodeGroup>
```bash Request
curl https://api.spairehq.com/v1/products/?page=1&limit=100 \
  -H "Authorization: Bearer $SPAIRE_OAT" \
  -H "Accept: application/json"
```
...
Spaire API has rate limits to ensure fair usage and maintain performance. The limits are as follows:
