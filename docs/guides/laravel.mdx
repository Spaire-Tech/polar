---
title: "Integrate Spaire with Laravel"
sidebarTitle: "Laravel"
description: "In this guide, we'll show you how to integrate Spaire with Laravel."
hidden: true
---
...
Spaire Laravel Example App
--------------------------------

We've created a simple example Laravel application that you can use as a reference.

[View Code on GitHub](https://github.com/spairehq/spaire-laravel)
...
To authenticate with Spaire, you need to create an access token, and supply it to Laravel using a `SPAIRE_API_KEY` environment variable.

You can create an organization access token from your organization settings.

Fetching Spaire Products for display
------------------------------------------
...
class ProductsController extends Controller
{
    ...
    {
        // Change from sandbox-api.spairehq.com -> api.spairehq.com when ready to go live
        // And don't forget to update the .env file with the correct SPAIRE_ORGANIZATION_ID and SPAIRE_WEBHOOK_SECRET
        $data = Http::get('https://sandbox-api.spairehq.com/v1/products', [
            'is_archived' => false,
        ]);
...
This endpoint will be responsible for creating a new checkout session, redirecting the user to the Spaire Checkout page & redirect back to a configured confirmation page.
...
class CheckoutController extends Controller
{
    public function handle(Request $request)
    {
        ...
        // Spaire will replace {CHECKOUT_ID} with the actual checkout ID upon a confirmed checkout
        $confirmationUrl = $request->getSchemeAndHttpHost() . '/confirmation?checkout_id={CHECKOUT_ID}';

        // Change from sandbox-api.spairehq.com -> api.spairehq.com when ready to go live
        // And don't forget to update the .env file with the correct SPAIRE_ORGANIZATION_ID and SPAIRE_WEBHOOK_SECRET
        $result = Http::withHeaders([
            'Authorization' => 'Bearer ' . env('SPAIRE_API_KEY'),
            'Content-Type' => 'application/json',
        ])->post('https://sandbox-api.spairehq.com/v1/checkouts/custom/', [
...
class ConfirmationController extends Controller
{
    public function handle(Request $request)
    {
        ...
        $data = Http::withHeaders([
            'Authorization' => 'Bearer ' . env('SPAIRE_API_KEY'),
            'Content-Type' => 'application/json',
        ])->get('https://sandbox-api.spairehq.com/v1/checkouts/custom/' . $request->query('checkout_id'));
...
return [
    'configs' => [
        [
             ...
             */
            'signing_secret' => env('SPAIRE_WEBHOOK_SECRET'),
            ...
            'signature_validator' => App\Handler\SpaireSignature::class,
...
Inside app/Handler/SpaireSignature.php, what we want to do is to validate that the request came from Spaire. Add the code to that file.

```php
// app/Handler/SpaireSignature.php
<?php
...
class SpaireSignature implements SignatureValidator
{
