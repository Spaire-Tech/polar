...
There are two ways to grant initial meter credits in Spaire:
...
- A Spaire account with an organization
- A meter created with **Sum** aggregation
- A product with the meter attached
- Webhooks enabled
- A Spaire access token for API calls
...
<Steps>
  <Step title="Navigate to Meters">
    In the Spaire dashboard sidebar, click on **Products** > **Meters**.
  </Step>
...
```typescript icon="square-js" title="app/api/webhook/spaire/route.ts"
...
<Steps>
  <Step title="Use the sandbox environment">
    Test your integration in Spaire's [sandbox environment](/integrate/sandbox) to avoid affecting production data.
  </Step>
