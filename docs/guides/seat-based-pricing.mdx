...
- Spaire organization with `seat_based_pricing_enabled` feature flag
- Spaire SDK installed (`npm install @spaire/sdk` or `pip install spaire-sdk`)
...
- Basic understanding of Spaire products and subscriptions
...
<Steps>
  <Step title="Navigate to Products">
    In the Spaire dashboard, go to **Products** and click **Create Product**.
  </Step>
...
```typescript
const subscriptionProduct = await spaire.products.create({
  name: "Team Pro Plan",
...
```typescript
const oneTimeProduct = await spaire.products.create({
...
```typescript
// Webhook handler
app.post('/webhooks/spaire', async (req, res) => {
...
async function getSeatInfo(params: { subscription_id?: string; order_id?: string }) {
  const { seats, available_seats, total_seats } =
    await spaire.customerSeats.list(params);
...
async function assignSeat(
  ...
  try {
    const seat = await spaire.customerSeats.assign({
      ...params,
      email: email,
...
async function handleClaimPage(token: string) {
  ...
  const claimInfo = await spaire.customerSeats.getClaimInfo({
...
async function claimSeat(token: string) {
  const { seat, customer_session_token } =
    await spaire.customerSeats.claim({
      invitation_token: token
    });

  // Store the customer session token
  // This allows immediate portal access
  localStorage.setItem('spaire_session', customer_session_token);
...
async function revokeSeat(seatId: string) {
  const revokedSeat = await spaire.customerSeats.revoke({
    seat_id: seatId
  });
...
async function addSeats(subscriptionId: string, newTotal: number) {
  ...
  const subscription = await spaire.subscriptions.update({
    id: subscriptionId,
    seats: newTotal
  });
...
async function reduceSeats(subscriptionId: string, newTotal: number) {
  const { seats } = await spaire.customerSeats.list({
...
async function getTeamSubscriptions(customerId: string) {
  const subs = await spaire.customerPortal.seats.listSubscriptions({
...
```typescript
await spaire.customerSeats.resend({ seat_id: seatId });
```
