...
description: "Learn how to automatically schedule subscription downgrades at the end of the current billing period using the Spaire API and Upstash QStash."
---
...
- A [Spaire API access token](/integrate/authentication)
...
```bash Terminal
# Spaire API credentials
SPAIRE_MODE="sandbox" # can be "production"
SPAIRE_ACCESS_TOKEN="spaire_pat_..."
...
const spaire = new Spaire({
  server: process.env.SPAIRE_MODE,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const subscription = await spaire.subscriptions.get({
      id: subscriptionId,
    });
...
async function handler(req: NextRequest) {
  try {
    ...
    const updatedSubscription = await spaire.subscriptions.update({
      id: subscriptionId,
      subscriptionUpdate: {
...
<Steps>
  <Step title="Test in Sandbox Environment">
    Use Spaire's [sandbox environment](/integrate/sandbox) to test without affecting production data.
