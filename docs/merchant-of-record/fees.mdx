...
All transactions on Spaire come with a small fee of 4% + 40¢ - applied to the entire transaction amount.

Spaire is currently built on Stripe, and we cover their 2.9% + 30¢ fee from ours. However, they impose a few additional fees for certain transactions that we need to pass on.
...
However, we continuously work to proactively reduce the rate of chargebacks across Spaire to be at or lower than industry standards.



Credit card networks impose monitoring programs, penalties and higher chargeback costs for sellers with high chargeback rates (~0.7%+). Since Spaire is the Merchant of Record, we therefore always monitor and proactively prevent our rate coming close to these thresholds.
...
While payouts may incur fees charged by our payout provider (Stripe), Spaire does not add any extra fees or markup. These are strictly Stripe’s fees, and Spaire does not profit from them.

In addition, Spaire offers manual withdrawals for developers. Keeping you in control of when to issue payouts.

*Unless you have a Spaire balance that you haven't withdrawn for several months, at which point we'll eventually need to trigger a payout on your behalf.*
